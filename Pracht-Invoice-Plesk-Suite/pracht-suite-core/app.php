...
final class PrachtSuite
{
    ...
    private static function ensureControlTables(): void
    {
        self::db()->exec('CREATE TABLE IF NOT EXISTS control_sites (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, name VARCHAR(190) NOT NULL, domain VARCHAR(255) NOT NULL, hosting VARCHAR(100) NULL, state VARCHAR(20) NOT NULL DEFAULT "Aktiv", connection VARCHAR(30) NOT NULL DEFAULT "Nicht verbunden", control_token VARCHAR(80) NOT NULL UNIQUE, production_domain VARCHAR(255) NULL, staging_domain VARCHAR(255) NULL, repository_url VARCHAR(500) NULL, project_status VARCHAR(30) NOT NULL DEFAULT "laufend", notes TEXT NULL, created_at DATETIME NOT NULL, updated_at DATETIME NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4');
        self::db()->exec('CREATE TABLE IF NOT EXISTS control_traffic (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY, site_id INT UNSIGNED NOT NULL, traffic_date DATE NOT NULL, page_views INT UNSIGNED NOT NULL DEFAULT 0, contact_events INT UNSIGNED NOT NULL DEFAULT 0, UNIQUE KEY unique_site_day(site_id,traffic_date), CONSTRAINT fk_control_traffic_site FOREIGN KEY (site_id) REFERENCES control_sites(id) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4');
        self::db()->exec('CREATE TABLE IF NOT EXISTS control_tasks (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, site_id INT UNSIGNED NULL, title VARCHAR(255) NOT NULL, category VARCHAR(60) NOT NULL DEFAULT "Website", priority VARCHAR(20) NOT NULL DEFAULT "normal", due_date DATE NULL, notes TEXT NULL, status VARCHAR(20) NOT NULL DEFAULT "offen", created_at DATETIME NOT NULL, completed_at DATETIME NULL, CONSTRAINT fk_control_tasks_site FOREIGN KEY (site_id) REFERENCES control_sites(id) ON DELETE SET NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4');
    }
    ...
    private static function controlPost(array $user, string $action): void
    {
        ...
        if ($action === 'create_control_site') { self::createControlSite((int) $user['id']); self::flash('Kundenwebsite wurde für Pracht Control angelegt.'); return; }
        if ($action === 'set_site_state') { self::setControlSiteState((int) $user['id']); self::flash('Website-Status wurde gespeichert.'); return; }
        if ($action === 'create_control_task') { self::createControlTask((int) $user['id']); self::flash('Aufgabe wurde angelegt.'); return; }
        if ($action === 'toggle_control_task') { self::toggleControlTask((int) $user['id']); self::flash('Aufgabenstatus aktualisiert.'); return; }
    ...
    private static function createControlSite(int $userId): void
    {
        self::ensureControlTables(); $name=trim((string)($_POST['site_name']??'')); $domain=trim((string)($_POST['site_domain']??''));
        if($name===''||$domain==='') throw new RuntimeException('Name und Domain der Kundenwebsite werden benötigt.');
        self::exec('INSERT INTO control_sites (name,domain,hosting,control_token,production_domain,created_at) VALUES (?,?,?,?,?,NOW())',[$name,$domain,trim((string)($_POST['site_hosting']??''))?:'Plesk',bin2hex(random_bytes(24)),$domain]);
        self::audit(null,$userId,'control_site_created',['name'=>$name,'domain'=>$domain]);
    }

    private static function setControlSiteState(int $userId): void
    {
        self::ensureControlTables(); $siteId=(int)($_POST['site_id']??0); $state=(string)($_POST['state']??'Aktiv');
        if(!in_array($state,['Aktiv','Wartung','404'],true)) throw new RuntimeException('Ungültiger Website-Status.');
        self::exec('UPDATE control_sites SET state=?,updated_at=NOW() WHERE id=?',[$state,$siteId]); self::audit(null,$userId,'control_site_state',['site_id'=>$siteId,'state'=>$state]);
    }

    private static function createControlTask(int $userId): void
    {
        self::ensureControlTables(); $title=trim((string)($_POST['task_title']??'')); if($title==='') throw new RuntimeException('Bitte einen Aufgabentitel eingeben.');
        $priority=(string)($_POST['task_priority']??'normal'); if(!in_array($priority,['niedrig','normal','dringend'],true))$priority='normal'; $due=self::validDate((string)($_POST['task_due_date']??''));
        self::exec('INSERT INTO control_tasks (site_id,title,category,priority,due_date,notes,created_at) VALUES (?,?,?,?,?,?,NOW())',[(int)($_POST['task_site_id']??0)?:null,$title,trim((string)($_POST['task_category']??'Website'))?:'Website',$priority,$due,trim((string)($_POST['task_notes']??''))?:null]); self::audit(null,$userId,'control_task_created',['title'=>$title]);
    }

    private static function toggleControlTask(int $userId): void
    {
        self::ensureControlTables(); $taskId=(int)($_POST['task_id']??0); $task=self::one('SELECT * FROM control_tasks WHERE id=?',[$taskId]); if(!$task) throw new RuntimeException('Aufgabe nicht gefunden.'); $next=$task['status']==='erledigt'?'offen':'erledigt';
        self::exec('UPDATE control_tasks SET status=?,completed_at=? WHERE id=?',[$next,$next==='erledigt'?date('Y-m-d H:i:s'):null,$taskId]); self::audit(null,$userId,'control_task_toggled',['task_id'=>$taskId,'status'=>$next]);
    }
    ...
    private static function controlPage(array $user): void
    {
        ...
        $activity = self::all('SELECT a.*,s.name AS site_name FROM audit_logs a LEFT JOIN control_sites s ON JSON_UNQUOTE(JSON_EXTRACT(a.context_json,"$.site_id"))=CAST(s.id AS CHAR) ORDER BY a.created_at DESC LIMIT 10');
        $trafficTotal = array_sum(array_map(fn($site)=>(int)$site['traffic_30'],$sites)); $contactTotal = array_sum(array_map(fn($site)=>(int)$site['contacts_30'],$sites)); $openTasks=count(array_filter($tasks,fn($task)=>$task['status']!=='erledigt'));
        self::layout('Pracht Control · Rechnungen', 'control', $user, function () use ($organizations, $totals, $users, $sites, $tasks, $activity, $trafficTotal, $contactTotal, $openTasks) { ?>
          <header class="control-header"><a class="control-brand" href="?"><span>P</span><b>Pracht Control<small>Invoice Suite</small></b></a><div class="control-status"><i></i> Live verbunden · <?= count($organizations) ?> Mandant<?= count($organizations) === 1 ? '' : 'en' ?></div><details class="user-menu"><summary><?= self::e($user['name']) ?></summary><a href="?action=logout">Abmelden</a></details></header>
          <section class="hero control-hero"><div><p class="eyebrow">Pracht Control / Invoice</p><h1>Rechnungen.<br><em>Im Überblick.</em></h1></div><p>Mandanten, Zugänge und echte Rechnungswerte zentral steuern - ohne zwischen einzelnen Portalen zu springen.</p></section>
          <?= self::flashHtml() ?>
          ...
          <section class="control-section"><div class="section-title"><div><p class="eyebrow">Pracht Control / Kundenwebsites</p><h2>Website-Portfolio.</h2></div><span><?= count($sites) ?> Website<?= count($sites) === 1 ? '' : 's' ?></span></div><section class="metrics control-metrics"><article><span>Besuche / 30 Tage</span><strong><?= number_format($trafficTotal,0,',','.') ?></strong><small>aus verbundenen Websites</small></article><article><span>Kontakt-Events</span><strong><?= number_format($contactTotal,0,',','.') ?></strong><small>Formulare, Telefon, E-Mail</small></article><article><span>Offene Aufgaben</span><strong><?= $openTasks ?></strong><small>über alle Kunden</small></article><article><span>Live-Control</span><strong><?= count(array_filter($sites,fn($site)=>$site['connection']==='Verbunden')) ?>/<?= count($sites) ?></strong><small>Control Kit verbunden</small></article></section><div class="grid two"><article class="card"><p class="eyebrow">Neue Kundenwebsite</p><h2>Website<br><em>anlegen.</em></h2><form method="post" class="form"><input type="hidden" name="csrf" value="<?= self::csrf() ?>"><input type="hidden" name="action" value="create_control_site"><input type="hidden" name="return_to" value=""><label>Name<input name="site_name" required placeholder="z. B. CK Eventcenter"></label><label>Produktiv-Domain<input name="site_domain" required placeholder="kunde.de oder kunde.hosting.tld"></label><label>Hosting<input name="site_hosting" placeholder="Plesk, Netlify, ..."></label><button class="button">Website anlegen <b>↗</b></button></form></article><article class="card dark"><p class="eyebrow">Website steuern</p><h2>Aktiv, Wartung<br><em>oder 404.</em></h2><p class="card-copy">Der Status wird hier zentral dokumentiert. Für eine Live-Schaltung wird beim jeweiligen Kunden anschließend das Control Kit verbunden.</p><ol class="steps"><li><b>01</b> Kundenwebsite anlegen</li><li><b>02</b> Control Kit in die Website einsetzen</li><li><b>03</b> Traffic und Status zentral nutzen</li></ol></article></div><section class="card table-card"><div class="table scroll"><table><thead><tr><th>Website</th><th>Hosting</th><th>Status</th><th>Besuche / 30 T.</th><th>Kontakte</th><th>Schaltung</th></tr></thead><tbody><?php foreach($sites as $site): ?><tr><td><b><?= self::e($site['name']) ?></b><small><?= self::e($site['production_domain'] ?: $site['domain']) ?></small></td><td><?= self::e($site['hosting']) ?><small><?= self::e($site['connection']) ?></small></td><td><span class="status <?= $site['state']==='Aktiv' ? 'ok' : ($site['state']==='404'?'off':'draft') ?>"><?= self::e($site['state']) ?></span></td><td><?= number_format((int)$site['traffic_30'],0,',','.') ?></td><td><?= number_format((int)$site['contacts_30'],0,',','.') ?></td><td><form method="post" class="site-state"><input type="hidden" name="csrf" value="<?= self::csrf() ?>"><input type="hidden" name="action" value="set_site_state"><input type="hidden" name="site_id" value="<?= (int)$site['id'] ?>"><button name="state" value="Aktiv" class="<?= $site['state']==='Aktiv'?'is-active':'' ?>">Aktiv</button><button name="state" value="Wartung" class="<?= $site['state']==='Wartung'?'is-active':'' ?>">Wartung</button><button name="state" value="404" class="danger <?= $site['state']==='404'?'is-active':'' ?>">404</button></form></td></tr><?php endforeach; if(!$sites): ?><tr><td colspan="6" class="empty">Noch keine Kundenwebsite angelegt.</td></tr><?php endif; ?></tbody></table></div></section></section>
          <section class="grid two control-work-grid"><article class="card"><p class="eyebrow">Aufgaben</p><h2>Was steht<br><em>an?</em></h2><form method="post" class="form"><input type="hidden" name="csrf" value="<?= self::csrf() ?>"><input type="hidden" name="action" value="create_control_task"><label>Kunde<select name="task_site_id"><option value="">Ohne Zuordnung</option><?php foreach($sites as $site): ?><option value="<?= (int)$site['id'] ?>"><?= self::e($site['name']) ?></option><?php endforeach; ?></select></label><label>Aufgabe<input name="task_title" required placeholder="Was muss erledigt werden?"></label><div class="form-grid"><label>Bereich<select name="task_category"><option>Website</option><option>Design</option><option>Technik</option><option>Domain</option><option>Support</option></select></label><label>Priorität<select name="task_priority"><option value="normal">normal</option><option value="dringend">dringend</option><option value="niedrig">niedrig</option></select></label></div><label>Fällig am<input type="date" name="task_due_date"></label><button class="button">Aufgabe speichern <b>↗</b></button></form></article><article class="card"><p class="eyebrow">Arbeitsliste</p><h2>Offene<br><em>Aufgaben.</em></h2><div class="task-list"><?php foreach($tasks as $task): ?><article class="task <?= $task['status']==='erledigt'?'is-done':'' ?>"><form method="post"><input type="hidden" name="csrf" value="<?= self::csrf() ?>"><input type="hidden" name="action" value="toggle_control_task"><input type="hidden" name="task_id" value="<?= (int)$task['id'] ?>"><button aria-label="Aufgabenstatus ändern">✓</button></form><div><b><?= self::e($task['title']) ?></b><small><?= self::e($task['site_name'] ?: $task['category']) ?> · <?= $task['due_date'] ? self::date($task['due_date']) : 'ohne Termin' ?></small></div><em class="priority <?= self::e($task['priority']) ?>"><?= self::e($task['priority']) ?></em></article><?php endforeach; if(!$tasks): ?><p class="empty">Noch keine Aufgaben angelegt.</p><?php endif; ?></div></article></section>
          <section class="card table-card control-activity"><div class="card-head"><div><p class="eyebrow">Kontrollprotokoll</p><h2>Letzte Aktivitäten</h2></div></div><ol><?php foreach($activity as $entry): ?><li><span></span><div><b><?= self::e(str_replace('_',' ',(string)$entry['event'])) ?></b><small><?= self::date($entry['created_at']) ?> · <?= self::e($entry['site_name'] ?: 'Pracht Control') ?></small></div></li><?php endforeach; if(!$activity): ?><li class="empty">Noch keine Aktivitäten.</li><?php endif; ?></ol></section>
        <?php });
    }
    ...
    private static function invoiceForm(array $org, int $orgId): void
    {
        ...
        $selected = $invoice['customer_id'] ? self::one('SELECT * FROM customers WHERE id=? AND organization_id=?', [(int) $invoice['customer_id'], $orgId]) : null;
        $customer = $selected ?: ['id' => '', 'name' => '', 'email' => '', 'street' => '', 'postal_code' => '', 'city' => '', 'country' => 'Deutschland']; ?>
        <section class="page-head invoice-page-head"><div><p class="eyebrow"><?= $invoice['id'] ? 'Entwurf bearbeiten' : 'Neue Rechnung' ?></p><h1>Rechnung<br><em>erstellen.</em></h1></div><div class="head-meta"><span>Automatisch gespeichert:</span><b>Kundendaten &amp; Positionen</b></div><a class="ghost" href="?view=documents">Schließen ↗</a></section>
        <form method="post" class="invoice-form invoice-workspace" id="invoice-form"><input type="hidden" name="csrf" value="<?= self::csrf() ?>"><input type="hidden" name="return_to" value="view=documents"><input type="hidden" name="invoice_id" value="<?= (int) $invoice['id'] ?>">
          <div class="invoice-editor">
            <section class="card form-card recipient-card"><div class="card-head"><div><p class="eyebrow">01 / Rechnungsempfänger</p><h2>Kunde direkt<br><em>eintragen.</em></h2></div></div>
              <div class="customer-picker"><label>Gespeicherten Kunden übernehmen<select id="customer-picker" name="customer_id"><option value="">Neue Kundendaten eingeben</option><?php foreach ($customers as $entry): ?><option value="<?= (int) $entry['id'] ?>" data-name="<?= self::e($entry['name']) ?>" data-email="<?= self::e($entry['email']) ?>" data-street="<?= self::e($entry['street']) ?>" data-postal="<?= self::e($entry['postal_code']) ?>" data-city="<?= self::e($entry['city']) ?>" data-country="<?= self::e($entry['country']) ?>" <?= (int) $invoice['customer_id'] === (int) $entry['id'] ? 'selected' : '' ?>><?= self::e($entry['name']) ?></option><?php endforeach; ?></select><small>Oder einfach unten ausfüllen - der Kunde wird beim Speichern automatisch angelegt.</small></label></div>
              <div class="form-grid"><label>Name / Firma<input id="customer-name" name="customer_name" value="<?= self::e($customer['name']) ?>" required placeholder="z. B. Mustermann GmbH"></label><label>E-Mail <small>optional</small><input id="customer-email" type="email" name="customer_email" value="<?= self::e($customer['email']) ?>" placeholder="rechnung@kunde.de"></label><label>Straße &amp; Hausnummer<input id="customer-street" name="customer_street" value="<?= self::e($customer['street']) ?>" required></label><div class="form-grid split"><label>PLZ<input id="customer-postal" name="customer_postal_code" value="<?= self::e($customer['postal_code']) ?>" required></label><label>Ort<input id="customer-city" name="customer_city" value="<?= self::e($customer['city']) ?>" required></label></div><label>Land<input id="customer-country" name="customer_country" value="<?= self::e($customer['country'] ?: 'Deutschland') ?>"></label></div>
            </section>
            <section class="card form-card"><div class="card-head"><div><p class="eyebrow">02 / Positionen</p><h2>Leistung &amp;<br><em>Betrag.</em></h2></div><button class="ghost" type="button" id="add-row">+ Position</button></div><div class="positions"><div class="position-head"><span>Beschreibung</span><span>Menge</span><span>Einzelpreis netto</span><span>USt.</span><span>Gesamt netto</span><span></span></div><div id="position-rows"><?php foreach ($items as $item): ?><div class="position-row"><input name="item_title[]" value="<?= self::e($item['title']) ?>" placeholder="Beschreibung der Leistung" required><input class="item-qty" name="item_qty[]" type="number" min="0.001" step="0.001" value="<?= self::e((string) $item['quantity']) ?>" required><input class="item-price" name="item_price[]" type="number" min="0" step="0.01" value="<?= self::e((string) $item['unit_price']) ?>" required><select class="item-tax" name="item_tax[]"><option value="19" <?= (float) $item['tax_rate'] === 19.0 ? 'selected' : '' ?>>19 %</option><option value="7" <?= (float) $item['tax_rate'] === 7.0 ? 'selected' : '' ?>>7 %</option><option value="0" <?= (float) $item['tax_rate'] === 0.0 ? 'selected' : '' ?>>0 %</option></select><output>0,00 €</output><button type="button" class="remove-row" aria-label="Position entfernen">×</button></div><?php endforeach; ?></div></div></section>
            <section class="card form-card invoice-texts"><div class="card-head"><div><p class="eyebrow">03 / Texte</p><h2>Hinweis &amp;<br><em>Zahlung.</em></h2></div></div><div class="form-grid one"><label>Hinweis auf der Rechnung<textarea name="note" rows="3" placeholder="z. B. Leistungszeitraum oder Projektbezug"><?= self::e($invoice['note']) ?></textarea></label><label>Abschlusszeile<textarea name="footer" rows="3"><?= self::e($invoice['footer']) ?></textarea></label></div></section>
          </div>
          <aside class="invoice-sidebar"><section class="issue-card"><p class="eyebrow">Rechnungsdaten</p><div class="form-grid one"><label>Rechnungsdatum<input type="date" name="issue_date" value="<?= self::e($invoice['issue_date']) ?>" required></label><label>Leistungsdatum<input type="date" name="service_date" value="<?= self::e($invoice['service_date']) ?>"></label><label>Zahlbar bis<input type="date" name="due_date" value="<?= self::e($invoice['due_date']) ?>"></label></div><div class="sidebar-rule"></div><p class="eyebrow">Zusammenfassung</p><div class="totals"><span>Netto <b id="net-total">0,00 €</b></span><span>Umsatzsteuer <b id="tax-total">0,00 €</b></span><strong>Gesamt <b id="gross-total">0,00 €</b></strong></div><p class="issue-hint">Finale Rechnungen erhalten eine fortlaufende Nummer und können danach nicht mehr verändert werden.</p><div class="issue-actions"><button class="ghost" type="submit" name="action" value="save_invoice">Entwurf speichern</button><button class="button" type="submit" name="action" value="issue_invoice">Rechnung final ausstellen <b>↗</b></button></div></section></aside>
        </form><template id="position-template"><div class="position-row"><input name="item_title[]" placeholder="Beschreibung der Leistung" required><input class="item-qty" name="item_qty[]" type="number" min="0.001" step="0.001" value="1" required><input class="item-price" name="item_price[]" type="number" min="0" step="0.01" required><select class="item-tax" name="item_tax[]"><option value="19">19 %</option><option value="7">7 %</option><option value="0">0 %</option></select><output>0,00 €</output><button type="button" class="remove-row" aria-label="Position entfernen">×</button></div></template>
    <?php }

    private static function customersPage(int $orgId): void
    {
        $customers = self::all('SELECT * FROM customers WHERE organization_id=? ORDER BY name', [$orgId]); $editId = (int) ($_GET['edit'] ?? 0); $editing = $editId ? self::one('SELECT * FROM customers WHERE id=? AND organization_id=?', [$editId, $orgId]) : null; $form = $editing ?: ['id' => 0, 'name' => '', 'email' => '', 'street' => '', 'postal_code' => '', 'city' => '', 'country' => 'Deutschland']; ?>
        <section class="page-head customer-page-head"><div><p class="eyebrow">Rechnungsempfänger</p><h1>Deine<br><em>Kunden.</em></h1><p class="page-copy">Alle Empfänger an einem Ort. Einmal gespeichert, beim nächsten Rechnungsschritt direkt wieder auswählbar.</p></div><div class="page-stat"><strong><?= count($customers) ?></strong><span>gespeicherte<br>Empfänger</span></div></section>
        <section class="grid two customer-layout"><article class="card customer-form-card"><div class="card-head"><div><p class="eyebrow">01 / <?= $editing ? 'Kontakt bearbeiten' : 'Neuer Kontakt' ?></p><h2>Empfänger<br><em><?= $editing ? 'aktualisieren.' : 'anlegen.' ?></em></h2></div><span class="card-index"><?= $editing ? '↗' : '+' ?></span></div><p class="card-copy"><?= $editing ? 'Ändere die Daten und speichere den Kontakt erneut.' : 'Trage die Daten einmal ein. Das Portal übernimmt sie automatisch in deine nächsten Rechnungen.' ?></p><form method="post" class="form"><input type="hidden" name="csrf" value="<?= self::csrf() ?>"><input type="hidden" name="action" value="save_customer"><input type="hidden" name="return_to" value="view=customers"><input type="hidden" name="customer_id" value="<?= (int) $form['id'] ?>"><div class="form-section-label">Kontakt</div><label>Firmenname / Name<input name="name" value="<?= self::e($form['name']) ?>" required placeholder="z. B. Mustermann GmbH"></label><label>E-Mail <small>optional</small><input type="email" name="email" value="<?= self::e($form['email']) ?>" placeholder="rechnung@kunde.de"></label><div class="form-section-label">Anschrift</div><label>Straße &amp; Hausnummer<input name="street" value="<?= self::e($form['street']) ?>" required placeholder="Musterstraße 12"></label><div class="form-grid split"><label>PLZ<input name="postal_code" value="<?= self::e($form['postal_code']) ?>" required placeholder="59192"></label><label>Ort<input name="city" value="<?= self::e($form['city']) ?>" required placeholder="Bergkamen"></label></div><label>Land<input name="country" value="<?= self::e($form['country'] ?: 'Deutschland') ?>"></label><div class="customer-form-actions"><button class="button"><?= $editing ? 'Kontakt aktualisieren' : 'Kunde speichern' ?> <b>↗</b></button><?php if ($editing): ?><a class="ghost" href="?view=customers">Abbrechen</a><?php endif; ?></div></form></article><article class="card customer-directory-card"><div class="card-head"><div><p class="eyebrow">02 / Gespeichert</p><h2>Deine<br><em>Empfänger.</em></h2></div><span class="directory-count"><?= count($customers) ?></span></div><div class="customer-list"><?php foreach ($customers as $customer): $name = trim((string) $customer['name']); $initial = mb_strtoupper(mb_substr($name, 0, 1)); ?><article class="customer-entry"><div class="customer-avatar"><?= self::e($initial ?: '•') ?></div><div class="customer-entry-body"><div class="customer-entry-top"><b><?= self::e($name) ?></b><span class="customer-chip">Kontakt</span></div><p><?= self::e($customer['street']) ?><br><?= self::e(trim(($customer['postal_code'] ?? '') . ' ' . ($customer['city'] ?? ''))) ?></p><div class="customer-meta"><span>✉ <?= self::e($customer['email'] ?: 'Keine E-Mail hinterlegt') ?></span><a class="customer-edit" href="?view=customers&amp;edit=<?= (int) $customer['id'] ?>">Bearbeiten ↗</a></div></div></article><?php endforeach; if (!$customers): ?><div class="customer-empty"><span>+</span><b>Noch keine Empfänger.</b><p>Lege deinen ersten Kontakt links an.</p></div><?php endif; ?></div></article></section>
    <?php }
    ...
    private static function saveCustomer(int $orgId): void
    {
        ...
        $email = trim((string) ($_POST['email'] ?? '')); $country = trim((string) ($_POST['country'] ?? 'Deutschland')) ?: 'Deutschland'; $customerId = (int) ($_POST['customer_id'] ?? 0);
        if ($customerId) {
            $existing = self::one('SELECT id FROM customers WHERE id=? AND organization_id=?', [$customerId, $orgId]);
            if (!$existing) throw new RuntimeException('Kontakt nicht gefunden.');
            self::exec('UPDATE customers SET name=?,email=?,street=?,postal_code=?,city=?,country=?,updated_at=NOW() WHERE id=? AND organization_id=?', [$name, $email ?: null, $street, $postal, $city, $country, $customerId, $orgId]);
            return;
        }
        self::exec('INSERT INTO customers (organization_id,name,email,street,postal_code,city,country,created_at) VALUES (?,?,?,?,?,?,?,NOW())', [$orgId, $name, $email ?: null, $street, $postal, $city, $country]);
    }
    ...
    private static function saveInvoice(int $orgId, int $userId, bool $issue): int
    {
        ...
        self::db()->beginTransaction();
        try {
            $customer = self::resolveInvoiceCustomer($orgId);
    ...
    private static function resolveInvoiceCustomer(int $orgId): array
    {
        $name = trim((string) ($_POST['customer_name'] ?? ''));
        $email = strtolower(trim((string) ($_POST['customer_email'] ?? '')));
        $street = trim((string) ($_POST['customer_street'] ?? ''));
        $postal = trim((string) ($_POST['customer_postal_code'] ?? ''));
        $city = trim((string) ($_POST['customer_city'] ?? ''));
        $country = trim((string) ($_POST['customer_country'] ?? 'Deutschland')) ?: 'Deutschland';
        if ($name === '' || $street === '' || $postal === '' || $city === '') throw new RuntimeException('Bitte Name und vollständige Anschrift des Rechnungsempfängers eingeben.');
        if ($email !== '' && !filter_var($email, FILTER_VALIDATE_EMAIL)) throw new RuntimeException('Die E-Mail-Adresse des Kunden ist nicht gültig.');
        $selectedId = (int) ($_POST['customer_id'] ?? 0);
        $customer = $selectedId ? self::one('SELECT * FROM customers WHERE id=? AND organization_id=? FOR UPDATE', [$selectedId, $orgId]) : null;
        if (!$customer) $customer = self::one('SELECT * FROM customers WHERE organization_id=? AND LOWER(name)=LOWER(?) AND LOWER(street)=LOWER(?) AND postal_code=? AND LOWER(city)=LOWER(?) LIMIT 1 FOR UPDATE', [$orgId, $name, $street, $postal, $city]);
        if ($customer) {
            self::exec('UPDATE customers SET name=?,email=?,street=?,postal_code=?,city=?,country=?,updated_at=NOW() WHERE id=? AND organization_id=?', [$name, $email ?: null, $street, $postal, $city, $country, $customer['id'], $orgId]);
            $customer = self::one('SELECT * FROM customers WHERE id=? AND organization_id=?', [(int) $customer['id'], $orgId]);
        } else {
            self::exec('INSERT INTO customers (organization_id,name,email,street,postal_code,city,country,created_at,updated_at) VALUES (?,?,?,?,?,?,?,NOW(),NOW())', [$orgId, $name, $email ?: null, $street, $postal, $city, $country]);
            $customer = self::one('SELECT * FROM customers WHERE id=? AND organization_id=?', [(int) self::db()->lastInsertId(), $orgId]);
        }
        if (!$customer) throw new RuntimeException('Kundendaten konnten nicht gespeichert werden.');
        return $customer;
    }

    private static function deleteDraftInvoice(int $orgId, int $userId, int $invoiceId): void
    {
        $invoice = self::one('SELECT id,status FROM invoices WHERE id=? AND organization_id=?', [$invoiceId, $orgId]);
        if (!$invoice) throw new RuntimeException('Entwurf nicht gefunden.');
        if ($invoice['status'] !== 'draft') throw new RuntimeException('Final ausgestellte Rechnungen bleiben als Beleg erhalten. Erstelle stattdessen eine bearbeitbare Kopie.');
        self::exec('DELETE FROM invoices WHERE id=? AND organization_id=? AND status="draft"', [$invoiceId, $orgId]);
    ...
    private static function copyInvoiceToDraft(int $orgId, int $userId, int $invoiceId): int
    {
        ...
        self::db()->beginTransaction();
        try {
            self::exec('INSERT INTO invoices (organization_id,customer_id,status,issue_date,service_date,due_date,note,footer,net_total,tax_total,gross_total,created_at,updated_at) VALUES (?,?,"draft",?,?,?,?,?,?,?,?,NOW(),NOW())', [$orgId, $invoice['customer_id'], date('Y-m-d'), $invoice['service_date'], date('Y-m-d', strtotime('+14 days')), $invoice['note'], $invoice['footer'], $invoice['net_total'], $invoice['tax_total'], $invoice['gross_total']]);
            $copyId = (int) self::db()->lastInsertId();
            foreach ($items as $item) self::exec('INSERT INTO invoice_items (invoice_id,position_no,title,quantity,unit_price,tax_rate,net_total,tax_total,gross_total) VALUES (?,?,?,?,?,?,?,?,?)', [$copyId, $item['position_no'], $item['title'], $item['quantity'], $item['unit_price'], $item['tax_rate'], $item['net_total'], $item['tax_total'], $item['gross_total']]);
            self::audit($orgId, $userId, 'invoice_copied_to_draft', ['source_invoice_id' => $invoiceId, 'draft_invoice_id' => $copyId]);
            self::db()->commit();
            return $copyId;
    ...
    private static function generatePdf(int $orgId, int $invoiceId): string
    {
        ...
        if ($y < 210) { $page=$pdf->newPage(); $y=705; }
        $noteY=$y-28; if (!empty($data['note'])) { $pdf->text($page,42,$noteY,8,'HINWEIS',$muted,'bold'); $pdf->wrapped($page,42,$noteY-16,260,(string)$data['note'],8,$ink); }
        ...
        $pdf->line($page,42,78,553,78,$rule); $pdf->wrapped($page,42,60,230,$footer ?: 'Vielen Dank für Ihr Vertrauen.',7,$muted); $pdf->block($page,308,60,$bank,6.5,$muted); $pdf->block($page,442,60,$legal,6.5,$muted);
        $filename = 'rechnung-' . preg_replace('/[^A-Za-z0-9_-]+/', '-', (string)$data['number']) . '.pdf'; $path = self::storagePath('pdf/' . $orgId . '-' . $invoiceId . '.pdf'); $pdfOutput = $pdf->output(); $pdfOutput = str_replace(['/BaseFont /Helvetica >>','/BaseFont /Helvetica-Bold >>'], ['/BaseFont /Helvetica /Encoding /WinAnsiEncoding >>','/BaseFont /Helvetica-Bold /Encoding /WinAnsiEncoding >>'], $pdfOutput); self::ensureStorage(); $written = @file_put_contents($path, $pdfOutput); if ($written !== false) self::exec('UPDATE invoices SET pdf_filename=?,pdf_hash=? WHERE id=?',[$filename,hash('sha256',$pdfOutput),$invoiceId]); return $pdfOutput;
    }
...
body{background:radial-gradient(circle at 88% -10%,color-mix(in srgb,var(--accent) 13%,transparent),transparent 34rem),linear-gradient(120deg,#f6f4ee 0%,#ece9e0 100%)}
.control-header{display:flex;align-items:center;justify-content:space-between;gap:16px;min-height:82px;padding:14px clamp(18px,4vw,60px);border-bottom:1px solid #1716141b;background:color-mix(in srgb,#f7f6f2 86%,transparent);backdrop-filter:blur(18px)}.control-brand{display:flex;align-items:center;gap:10px;color:var(--ink);text-decoration:none}.control-brand>span{display:grid;place-items:center;width:40px;height:40px;border-radius:12px;background:var(--accent);font-family:'Playfair Display',serif;font-size:20px;box-shadow:0 6px 14px #fa513940}.control-brand b{font-size:13px;letter-spacing:-.04em}.control-brand small{display:block;margin-top:2px;color:var(--muted);font-family:'DM Mono',monospace;font-size:8px;letter-spacing:.1em;text-transform:uppercase}.control-status{margin-left:auto;color:#514e48;font-family:'DM Mono',monospace;font-size:10px}.control-status i{display:inline-block;width:7px;height:7px;margin-right:6px;border-radius:50%;background:#39a969;box-shadow:0 0 0 4px #39a96922}.control-header .user-menu{margin-left:4px}
.portal-header{position:sticky;top:0;z-index:20;min-height:84px;background:color-mix(in srgb,#f7f6f2 88%,transparent);backdrop-filter:blur(18px);box-shadow:0 1px 0 #1716140d}
.portal-brand>span{width:42px;height:42px;border-radius:12px;box-shadow:inset 0 0 0 1px #1716141a,0 6px 14px color-mix(in srgb,var(--accent) 24%,transparent)}
.portal-brand b{font-size:13px;letter-spacing:-.03em}.portal-header nav{padding:4px;border:1px solid #1716141a;border-radius:999px;background:#fff8}.portal-header nav a{border-radius:999px;padding:10px 14px}.portal-header nav a:hover,.portal-header nav a.active{background:var(--ink);box-shadow:0 5px 12px #1716141a}
.user-menu{padding:9px 12px;border:1px solid #1716141a;border-radius:999px;background:#fff9}.user-menu[open]{border-radius:14px}.user-menu summary:after{content:'⌄';margin-left:8px;color:var(--muted)}
.hero,.invoice-hero,.page-head{padding-top:72px}.hero:before,.invoice-hero:before,.page-head:before{content:'';display:block;position:absolute;width:90px;height:90px;border:1px solid color-mix(in srgb,var(--accent) 55%,transparent);border-radius:50%;transform:translate(16px,-36px);pointer-events:none}.hero h1,.invoice-hero h1,.page-head h1{position:relative;z-index:1}.invoice-hero{padding-bottom:46px}.invoice-hero>div{max-width:720px}.invoice-hero p{font-size:15px}.metrics article{position:relative;overflow:hidden;background:linear-gradient(135deg,#fff 0%,#f3f0e8 100%);box-shadow:0 12px 22px #17161409}.metrics article:after{content:'';position:absolute;right:-24px;bottom:-31px;width:72px;height:72px;border:1px solid color-mix(in srgb,var(--accent) 42%,transparent);border-radius:50%}.metrics strong{font-size:clamp(31px,3.1vw,51px)}
.card{border-color:#17161426;border-radius:16px;box-shadow:0 12px 28px #1716140a}.card-head{border-color:#17161418}.grid{gap:22px}.table-card{overflow:hidden}.table-card .card-head{margin:0;padding:22px 26px}.table-card table{background:#fff9}.table-card th{background:#ece8de}.table-card tr:last-child td{border:0}.table-card tr:hover td{background:color-mix(in srgb,var(--accent) 5%,transparent)}
.invoice .card.accent{background:linear-gradient(135deg,#e4e0ff,#c9c1ff);color:#171614}.invoice .card.accent .eyebrow,.invoice .card.accent h2 em,.invoice .card.accent .steps b{color:#171614}.invoice .card.accent .button.light{border-color:#171614;color:#171614}.invoice .card.accent .button.light:hover{background:#171614;color:#fff}.invoice-actions form{display:inline;margin:0}.link-button.danger{color:#9d3027}.link-button.danger:hover{color:#6f1f18;text-decoration:underline}
.control-section{max-width:1380px;margin:64px auto 0;padding:0 34px}.section-title{display:flex;align-items:end;justify-content:space-between;margin:0 0 18px}.section-title h2{margin:8px 0 0;font-size:clamp(32px,4vw,58px);line-height:.82;letter-spacing:-.08em}.section-title>span{padding:7px 10px;border:1px solid #17161424;border-radius:999px;font-family:'DM Mono',monospace;font-size:9px}.control-section>.control-metrics{padding:0;margin-bottom:18px}.site-state{display:flex;gap:4px}.site-state button{min-height:30px;padding:0 9px;border:1px solid #1716142c;background:transparent;font-size:9px;font-weight:700;cursor:pointer}.site-state button:hover,.site-state button.is-active{background:#171614;color:#fff}.site-state button.danger.is-active{background:#9d3027;border-color:#9d3027}.control-work-grid{margin-top:22px}.task-list{margin:0 -22px -22px}.task{display:grid;grid-template-columns:28px 1fr auto;gap:10px;align-items:center;padding:14px 22px;border-top:1px solid #17161418}.task form{margin:0}.task form button{display:grid;place-items:center;width:23px;height:23px;border:1px solid #17161448;background:#fff;border-radius:50%;color:transparent;cursor:pointer}.task.is-done{opacity:.55}.task.is-done form button{background:#5b9770;color:#fff;border-color:#5b9770}.task b,.task small{display:block}.task b{font-size:12px}.task small{margin-top:4px;color:var(--muted);font-size:10px}.priority{font-family:'DM Mono',monospace;font-size:8px;font-style:normal;text-transform:uppercase}.priority.dringend{color:#a82e26}.priority.niedrig{color:#66806e}.control-activity{margin-top:22px}.control-activity ol{margin:0;padding:0;list-style:none}.control-activity li{display:flex;gap:11px;align-items:center;padding:13px 22px;border-top:1px solid #17161418}.control-activity li>span{width:7px;height:7px;border-radius:50%;background:var(--accent)}.control-activity b,.control-activity small{display:block}.control-activity b{text-transform:capitalize;font-size:11px}.control-activity small{margin-top:3px;color:var(--muted);font-size:10px}
.customer-layout{grid-template-columns:minmax(0,1.45fr) minmax(320px,.85fr);align-items:start}.customer-layout>.table-card{width:100%;min-width:0;max-width:none;margin:0;overflow:hidden}.customer-layout>.table-card .card-head{width:100%}.customer-layout .customer-list{width:100%;overflow:hidden}.customer-layout .customer-list article{width:100%;min-width:0;overflow-wrap:anywhere}.customer-layout .customer-list article p{overflow-wrap:anywhere}
.login{background:radial-gradient(circle at 14% 22%,#fa513940,transparent 27rem),radial-gradient(circle at 86% 85%,#766aff38,transparent 26rem),#151515}.login section{position:relative;overflow:hidden;border-radius:24px;border-color:#ffffff25;box-shadow:0 24px 80px #0006}.login section:before{content:'P/';position:absolute;right:-6px;top:-70px;font-family:'Playfair Display',serif;font-size:270px;font-style:italic;line-height:1;color:#fff05}.login h1,.login p,.login form,.login .eyebrow,.login small,.login .flash{position:relative;z-index:1}.login .button{border-radius:8px}
.head-meta{margin-left:auto;padding:11px 14px;border-left:2px solid var(--accent);background:#fff8;font-size:10px;line-height:1.55}.head-meta span,.head-meta b{display:block}.head-meta span{color:var(--muted)}.invoice-page-head{align-items:center}
.invoice-workspace{display:grid;grid-template-columns:minmax(0,1fr) 320px;gap:22px;align-items:start;padding-bottom:50px}.invoice-editor{min-width:0}.invoice-sidebar{position:sticky;top:108px}.invoice-workspace .form-card{margin:0 0 18px;max-width:none}.recipient-card{background:linear-gradient(145deg,#fff 0%,#f5f1e8 100%)}.customer-picker{padding:12px 14px;margin:0 0 16px;border:1px dashed #17161445;border-radius:12px;background:#ffffff9e}.customer-picker label{display:grid;gap:7px;color:var(--muted);font-family:'DM Mono',monospace;font-size:10px}.customer-picker small{font-family:Manrope,Arial,sans-serif;font-size:10px;line-height:1.4;color:#615e57}.issue-card{overflow:hidden;padding:22px;border:1px solid #171614;background:#1b1a18;color:#f4f1eb;border-radius:16px;box-shadow:0 18px 38px #17161422}.issue-card .eyebrow{color:#b9b2a7}.issue-card .form-grid label{color:#d0c9bd}.issue-card input{border-color:#ffffff28;background:#ffffff0d;color:#fff}.issue-card input::-webkit-calendar-picker-indicator{filter:invert(1)}.sidebar-rule{height:1px;margin:22px 0;background:#ffffff2b}.issue-card .totals{margin:12px 0 18px}.issue-card .totals span,.issue-card .totals strong{min-width:100%;color:#f4f1eb}.issue-card .totals span{padding:8px 0;border-bottom:1px solid #ffffff1f}.issue-card .totals strong{background:var(--accent);color:#171614;border-radius:8px}.issue-hint{margin:0 0 16px;color:#c2bbb0;font-size:10px;line-height:1.55}.issue-actions{display:grid;gap:8px}.issue-actions .ghost{justify-content:center;border-color:#ffffff47;color:#f4f1eb}.issue-actions .ghost:hover{background:#fff;color:#171614}.issue-actions .button{justify-content:space-between;border-radius:8px}.positions{border:1px solid #1716141f;border-radius:12px;overflow:hidden;background:#fff}.position-head{padding:12px 13px;background:#ebe6dc}.position-row{padding:9px 13px}.position-row:last-child{border-bottom:0}.position-row input,.position-row select{border-color:#d7d1c5;background:#fcfbf7;min-height:42px}.position-row output{font-size:12px}.remove-row{border-radius:50%;background:#fff2f0}.invoice-texts{background:#f0ece3}
 .customer-page-head .page-copy{max-width:430px;margin:18px 0 0;color:#57534c;line-height:1.6}.page-stat{display:flex;align-items:center;gap:12px;padding:10px 0 10px 16px;border-left:2px solid var(--accent);font-family:'DM Mono',monospace;text-transform:uppercase}.page-stat strong{font-family:Manrope,Arial,sans-serif;font-size:40px;letter-spacing:-.08em}.page-stat span{color:var(--muted);font-size:9px;line-height:1.45;letter-spacing:.1em}.customer-form-card,.customer-directory-card{min-width:0}.customer-form-card>.card-copy{margin:-6px 0 24px}.form-section-label{padding-top:4px;color:var(--accent);font-family:'DM Mono',monospace;font-size:9px;letter-spacing:.1em;text-transform:uppercase}.card-index,.directory-count{display:grid;place-items:center;width:32px;height:32px;border:1px solid #17161425;border-radius:50%;font-family:'DM Mono',monospace;font-size:11px}.directory-count{background:var(--ink);color:var(--paper);border-color:var(--ink)}.customer-directory-card{overflow:hidden;padding-bottom:0}.customer-directory-card .card-head{margin-bottom:0}.customer-list{margin:0 -22px}.customer-entry{display:flex;gap:14px;padding:20px 22px;border-top:1px solid #17161418;transition:background .2s ease}.customer-entry:hover{background:color-mix(in srgb,var(--accent) 6%,transparent)}.customer-avatar{display:grid;place-items:center;flex:0 0 42px;width:42px;height:42px;border-radius:50%;background:var(--accent);color:var(--ink);font-family:'Playfair Display',serif;font-size:21px}.customer-entry-body{min-width:0;flex:1}.customer-entry-top{display:flex;align-items:center;justify-content:space-between;gap:10px}.customer-entry-top b{font-size:14px;overflow-wrap:anywhere}.customer-chip{padding:4px 7px;border:1px solid #1716141f;border-radius:999px;color:var(--muted);font-family:'DM Mono',monospace;font-size:8px;text-transform:uppercase}.customer-entry p{margin:7px 0;color:#57534c;font-size:11px;line-height:1.5}.customer-meta{color:var(--muted);font-size:10px;overflow-wrap:anywhere}.customer-empty{padding:46px 22px;text-align:center;color:var(--muted)}.customer-empty>span{display:grid;place-items:center;width:42px;height:42px;margin:0 auto 12px;border:1px dashed #17161455;border-radius:50%;font-size:20px}.customer-empty b{display:block;color:var(--ink);font-size:13px}.customer-empty p{margin:6px 0 0;font-size:11px}.customer-layout{align-items:stretch}
...
@media(max-width:700px){.customer-page-head .page-copy{margin-top:14px;font-size:13px}.customer-page-head .page-stat{margin-top:20px;width:max-content}.customer-form-card>.card-copy{font-size:12px}.customer-entry{padding:16px 17px;gap:11px}.customer-avatar{flex-basis:36px;width:36px;height:36px;font-size:18px}.customer-entry-top{display:block}.customer-chip{display:inline-block;margin-top:7px}.customer-entry p{font-size:10px}.customer-meta{font-size:9px}.customer-list{margin:0 -17px}.customer-form-actions{align-items:stretch;flex-direction:column}.customer-form-actions .button,.customer-form-actions .ghost{width:100%}}
.customer-form-actions{display:flex;align-items:center;gap:10px}.customer-form-actions .button{flex:1}.customer-form-actions .ghost{justify-content:center}.customer-edit{margin-left:auto;color:var(--ink);font-size:10px;font-weight:800;text-decoration:none;white-space:nowrap}.customer-edit:hover{text-decoration:underline}
CSS; }
}
